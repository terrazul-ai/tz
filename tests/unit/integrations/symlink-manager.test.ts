...
describe('symlink-manager', () => {
  ...
  describe('createSymlinks', () => {
    ...
    describe('multi-tool routing', () => {
      ...
      it('should skip files for tools with no operational directories', async () => {
        const pkgRoot = path.join(agentModulesDir, '@user', 'gemini-pkg');
        const geminiAgentsDir = path.join(pkgRoot, 'gemini', 'agents');
        await fs.mkdir(geminiAgentsDir, { recursive: true });

        await fs.writeFile(path.join(geminiAgentsDir, 'agent.md'), '# Gemini Agent');

        const renderedFiles = [
          {
            pkgName: '@user/gemini-pkg',
            source: path.join(geminiAgentsDir, 'agent.md'),
            tool: 'gemini' as const, // gemini has no operational dirs
            isMcpConfig: false,
          },
        ];
        ...
        const result = await createSymlinks({
          projectRoot: tmpDir,
          packages: ['@user/gemini-pkg'],
          renderedFiles,
        });

        // No symlinks created (gemini has empty operational dirs)
        expect(result.created).toHaveLength(0);
      });
    });
  ...
  describe('exclusive mode', () => {
    ...
    it('should remove all non-target package symlinks when exclusive is true', async () => {
      // Setup: Create two Claude packages with agents
      const pkgARoot = path.join(agentModulesDir, '@user', 'pkg-a');
      const pkgBRoot = path.join(agentModulesDir, '@user', 'pkg-b');
      const pkgAAgentsDir = path.join(pkgARoot, 'claude', 'agents');
      const pkgBAgentsDir = path.join(pkgBRoot, 'claude', 'agents');
      ...
      // Now create new package with exclusive mode
      const newPkgRoot = path.join(agentModulesDir, '@user', 'new-pkg');
      const newPkgAgentsDir = path.join(newPkgRoot, 'claude', 'agents');
      await fs.mkdir(newPkgAgentsDir, { recursive: true });
      await fs.writeFile(path.join(newPkgAgentsDir, 'new-agent.md'), '# New Agent');

      const result = await createSymlinks({
        projectRoot: tmpDir,
        packages: ['@user/new-pkg'],
        renderedFiles: [
          {
            pkgName: '@user/new-pkg',
            source: path.join(newPkgAgentsDir, 'new-agent.md'),
            tool: 'claude' as const,
            isMcpConfig: false,
          },
        ],
      ...
      // Both old symlinks should be removed (not in target packages)
      expect(result.removed).toHaveLength(2);
      expect(result.removed.some((p) => p.includes('@user-pkg-a-agent-a.md'))).toBe(true);
      expect(result.removed.some((p) => p.includes('@user-pkg-b-agent-b.md'))).toBe(true);

      // Check final state
      agentsSymlinks = await fs.readdir(path.join(claudeDir, 'agents'));

      // New symlink should exist
      expect(agentsSymlinks).toContain('@user-new-pkg-new-agent.md');

      // Old symlinks should be removed
      expect(agentsSymlinks).not.toContain('@user-pkg-a-agent-a.md');
      expect(agentsSymlinks).not.toContain('@user-pkg-b-agent-b.md');
    });
  ...
  describe('codex tool support', () => {
    ...
    it('should NOT create agent symlinks for codex tool (only skills and commands supported)', async () => {
      const pkgRoot = path.join(agentModulesDir, '@user', 'qa-engineer');
      ...
      // Skill + command should be symlinked (codex supports skills and commands, not agents)
      expect(result.created).toHaveLength(2);

      // Verify skills directory exists in .codex/
      ...
      const codexAgentsExists = await fs
        ...
        .then(() => true)
        .catch(() => false);
      // Commands are routed to .codex/prompts/ (not .codex/commands/)
      const codexPromptsExists = await fs
        .access(path.join(codexDir, 'prompts'))
        .then(() => true)
        .catch(() => false);

      expect(codexSkillsExists).toBe(true);
      expect(codexAgentsExists).toBe(false);
      expect(codexPromptsExists).toBe(true);
    });
  ...
  describe('codex commands routing', () => {
    it('should route Codex commands to codexHome/prompts/ when codexHome is provided', async () => {
      ...
      await fs.writeFile(path.join(commandsDir, 'review.md'), '# Code Review Command');

      const renderedFiles = [
        {
          pkgName: '@user/qa-engineer',
          source: path.join(commandsDir, 'review.md'),
          tool: 'codex' as const,
          isMcpConfig: false,
        },
      ];
      ...
      // Verify symlink is in CODEX_HOME/prompts/ (Codex stores commands as "prompts")
      ...
      // Verify .codex/prompts/ was NOT created (commands go to CODEX_HOME only)
    ...
    it('should route Codex commands to .codex/prompts/ when codexHome is not provided', async () => {
      const codexDir = path.join(tmpDir, '.codex');
      await fs.mkdir(codexDir, { recursive: true });
      ...
      // Verify symlink is in .codex/prompts/ (Codex stores commands as "prompts")
      const codexPrompts = await fs.readdir(path.join(codexDir, 'prompts'));
      expect(codexPrompts).toContain('@user-qa-engineer-review.md');
    });

    it('should create both skills and commands symlinks for Codex packages', async () => {
      const customCodexHome = path.join(tmpDir, '.terrazul', 'codex-home');
      await fs.mkdir(path.join(customCodexHome, 'prompts'), { recursive: true });
      ...
      // Commands go to CODEX_HOME/prompts/ (user-level, Codex stores commands as "prompts")
      const codexHomePrompts = await fs.readdir(path.join(customCodexHome, 'prompts'));
      expect(codexHomePrompts).toContain('@user-qa-engineer-review.md');
    });

    it('should track commands in registry with codexHome path', async () => {
      const customCodexHome = path.join(tmpDir, '.terrazul', 'codex-home');
      await fs.mkdir(path.join(customCodexHome, 'prompts'), { recursive: true });
      ...
      await fs.writeFile(path.join(commandsDir, 'my-command.md'), '# My Command');

      const renderedFiles = [
        {
          pkgName: '@user/pkg',
          source: path.join(commandsDir, 'my-command.md'),
          tool: 'codex' as const,
          isMcpConfig: false,
        },
      ];
      ...
      // Registry key should include the codexHome path, with "prompts" as the on-disk dir
      const symlinkKey = '.terrazul/codex-home/prompts/@user-pkg-my-command.md';
      expect(registry.symlinks[symlinkKey]).toBeDefined();
      expect(registry.symlinks[symlinkKey].tool).toBe('codex');
    });
  });
